...
import { createImageVideoGeneration, IMAGE_VIDEO_MAX_BYTES, imageVideoContentUrl, parseImageVideoGeneration, parseImageVideoWorkspace, readImageVideoGeneration, readImageVideoWorkspace, validImageVideoCommand } from "./image-video-client";
...
describe("image + description closed client contract", () => {
  ...
  it("uses only same-origin closed paths and never retries POST", async () => {
    const fetch = vi.fn().mockResolvedValueOnce(Response.json({ ok: true, workspace })).mockResolvedValueOnce(Response.json({ ok: true, generation })).mockResolvedValueOnce(Response.json({ ok: true, generation })).mockRejectedValueOnce(new TypeError("network failure"));
    vi.stubGlobal("fetch", fetch);
    await readImageVideoWorkspace(scope); await createImageVideoGeneration(scope, command); await readImageVideoGeneration(scope, generation.generationRef);
