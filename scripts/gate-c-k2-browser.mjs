...
(async () => {
  ...
  try {
    ...
    const approvalInputs = await page.locator("section[aria-labelledby='approval-title'] input[type='text']").all();
    assert(approvalInputs.length === 8, "four independent approval and actor pairs are not rendered");
    for (const input of approvalInputs) {
      assert((await input.inputValue()) === "", "an approval field was auto-filled");
    }
    ...
    for (const [label, approvalRef] of approvals) {
      await page.getByLabel(`${label}审批引用`).fill(approvalRef);
      await page.getByLabel(`${label}审批人引用`).fill("actor-project-lead");
    }
